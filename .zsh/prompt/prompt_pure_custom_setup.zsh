...
prompt_pure_custom_human_time_to_var() {
...
# Stores (into prompt_pure_custom_cmd_exec_time) the execution
# time of the last command if set threshold was exceeded.
prompt_pure_custom_check_cmd_exec_time() {
	integer elapsed
	(( elapsed = EPOCHSECONDS - ${prompt_pure_custom_cmd_timestamp:-$EPOCHSECONDS} ))
	typeset -g prompt_pure_custom_cmd_exec_time=
	(( elapsed > ${PURE_CMD_MAX_EXEC_TIME:-5} )) && {
		prompt_pure_custom_human_time_to_var $elapsed "prompt_pure_custom_cmd_exec_time"
	}
}

prompt_pure_custom_set_title() {
	...
	local hostname=
	if [[ -n $prompt_pure_custom_state[username] ]]; then
...
prompt_pure_custom_preexec() {
	if [[ -n $prompt_pure_custom_git_fetch_pattern ]]; then
		# Detect when Git is performing pull/fetch, including Git aliases.
		local -H MATCH MBEGIN MEND match mbegin mend
		if [[ $2 =~ (git|hub)\ (.*\ )?($prompt_pure_custom_git_fetch_pattern)(\ .*)?$ ]]; then
			# We must flush the async jobs to cancel our git fetch in order
			# to avoid conflicts with the user issued pull / fetch.
			async_flush_jobs 'prompt_pure_custom'
		fi
	fi

	typeset -g prompt_pure_custom_cmd_timestamp=$EPOCHSECONDS

	# Shows the current directory and executed command in the title while a process is active.
	prompt_pure_custom_set_title 'ignore-escape' "$PWD:t: $2"
...
prompt_pure_custom_set_colors() {
	local color_temp key value
	for key value in ${(kv)prompt_pure_custom_colors}; do
		...
		case $? in
			1) # The current style is different from the one from zstyle.
				...
				prompt_pure_custom_colors[$key]=$color_temp ;;
			2) # No style is defined.
				prompt_pure_custom_colors[$key]=$prompt_pure_custom_colors_default[$key] ;;
		esac
	done
}

prompt_pure_custom_preprompt_render() {
	setopt localoptions noshwordsplit

	# Set color for Git branch/dirty status and change color if dirty checking has been delayed.
	local git_color=$prompt_pure_custom_colors[git:branch]
	local git_dirty_color=$prompt_pure_custom_colors[git:dirty]
	[[ -n ${prompt_pure_custom_git_last_dirty_check_timestamp+x} ]] && git_color=$prompt_pure_custom_colors[git:branch:cached]
	...
	if [[ -n $prompt_pure_custom_vcs_info[branch] ]]; then
		...
		preprompt_parts+=("$branch""%F{$git_dirty_color}"'${prompt_pure_custom_git_dirty}%f')
	fi
	...
	# Execution time.
	[[ -n $prompt_pure_custom_cmd_exec_time ]] && preprompt_parts+=('%F{$prompt_pure_custom_colors[execution_time]}${prompt_pure_custom_cmd_exec_time}%f')
	...
	if [[ $1 == precmd ]]; then
		...
		print
	elif [[ $prompt_pure_custom_last_prompt != $expanded_prompt ]]; then
		# Redraw the prompt.
		prompt_pure_custom_reset_prompt
	fi

	typeset -g prompt_pure_custom_last_prompt=$expanded_prompt
}

prompt_pure_custom_precmd() {
	# Check execution time and store it in a variable.
	prompt_pure_custom_check_cmd_exec_time
	unset prompt_pure_custom_cmd_timestamp

	# Shows the full path in the title.
	prompt_pure_custom_set_title 'expand-prompt' '%~'

	# Modify the colors if some have changed..
	prompt_pure_custom_set_colors

	# Perform async Git dirty check and fetch.
	prompt_pure_custom_async_tasks
	...
	prompt_pure_custom_reset_prompt_symbol

	# Print the preprompt.
	prompt_pure_custom_preprompt_render "precmd"
...
prompt_pure_custom_async_git_aliases() {
	setopt localoptions noshwordsplit
...
prompt_pure_custom_async_vcs_info() {
	setopt localoptions noshwordsplit
...
prompt_pure_custom_async_git_dirty() {
	setopt localoptions noshwordsplit
...
prompt_pure_custom_async_git_fetch() {
	setopt localoptions noshwordsplit
	...
	prompt_pure_custom_async_git_arrows
}
...
prompt_pure_custom_async_tasks() {
	...
	((!${prompt_pure_custom_async_init:-0})) && {
		async_start_worker "prompt_pure_custom" -u -n
		async_register_callback "prompt_pure_custom" prompt_pure_custom_async_callback
		typeset -g prompt_pure_custom_async_init=1
	}

	# Update the current working directory of the async worker.
	async_worker_eval "prompt_pure_custom" builtin cd -q $PWD

	typeset -gA prompt_pure_custom_vcs_info

	local -H MATCH MBEGIN MEND
	if [[ $PWD != ${prompt_pure_custom_vcs_info[pwd]}* ]]; then
		# Stop any running async jobs.
		async_flush_jobs "prompt_pure_custom"

		# Reset Git preprompt variables, switching working tree.
		unset prompt_pure_custom_git_dirty
		unset prompt_pure_custom_git_last_dirty_check_timestamp
		unset prompt_pure_custom_git_arrows
		unset prompt_pure_custom_git_fetch_pattern
		prompt_pure_custom_vcs_info[branch]=
		prompt_pure_custom_vcs_info[top]=
	fi
	unset MATCH MBEGIN MEND

	async_job "prompt_pure_custom" prompt_pure_custom_async_vcs_info

	# Only perform tasks inside a Git working tree.
	[[ -n $prompt_pure_custom_vcs_info[top] ]] || return

	prompt_pure_custom_async_refresh
}

prompt_pure_custom_async_refresh() {
	setopt localoptions noshwordsplit

	if [[ -z $prompt_pure_custom_git_fetch_pattern ]]; then
		# We set the pattern here to avoid redoing the pattern check until the
		# working three has changed. Pull and fetch are always valid patterns.
		typeset -g prompt_pure_custom_git_fetch_pattern="pull|fetch"
		async_job "prompt_pure_custom" prompt_pure_custom_async_git_aliases
	fi

	async_job "prompt_pure_custom" prompt_pure_custom_async_git_arrows

	# Do not preform `git fetch` if it is disabled or in home folder.
	if (( ${PURE_GIT_PULL:-1} )) && [[ $prompt_pure_custom_vcs_info[top] != $HOME ]]; then
		# Tell the async worker to do a `git fetch`.
		async_job "prompt_pure_custom" prompt_pure_custom_async_git_fetch
	fi

	# If dirty checking is sufficiently fast,
	# tell the worker to check it again, or wait for timeout.
	integer time_since_last_dirty_check=$(( EPOCHSECONDS - ${prompt_pure_custom_git_last_dirty_check_timestamp:-0} ))
	if (( time_since_last_dirty_check > ${PURE_GIT_DELAY_DIRTY_CHECK:-1800} )); then
		unset prompt_pure_custom_git_last_dirty_check_timestamp
		# Check check if there is anything to pull.
		async_job "prompt_pure_custom" prompt_pure_custom_async_git_dirty ${PURE_GIT_UNTRACKED_DIRTY:-1}
	fi
}

prompt_pure_custom_check_git_arrows() {
	setopt localoptions noshwordsplit
...
prompt_pure_custom_async_callback() {
	setopt localoptions noshwordsplit
	...
	case $job in
		\[async])
			...
			if [[ $code -eq 2 ]]; then
				...
				typeset -g prompt_pure_custom_async_init=0
			fi
			;;
		prompt_pure_custom_async_vcs_info)
			local -A info
			...
			if [[ $info[top] = $prompt_pure_custom_vcs_info[top] ]]; then
				# If the stored pwd is part of $PWD, $PWD is shorter and likelier
				# to be top-level, so we update pwd.
				if [[ $prompt_pure_custom_vcs_info[pwd] = ${PWD}* ]]; then
					prompt_pure_custom_vcs_info[pwd]=$PWD
				fi
			else
			...
			[[ -n $info[top] ]] && [[ -z $prompt_pure_custom_vcs_info[top] ]] && prompt_pure_custom_async_refresh

			# Always update branch and top-level.
			prompt_pure_custom_vcs_info[branch]=$info[branch]
			prompt_pure_custom_vcs_info[top]=$info[top]
			prompt_pure_custom_vcs_info[action]=$info[action]
		...
		prompt_pure_custom_async_git_aliases)
			if [[ -n $output ]]; then
				...
				prompt_pure_custom_git_fetch_pattern+="|$output"
			fi
			;;
		prompt_pure_custom_async_git_dirty)
			local prev_dirty=$prompt_pure_custom_git_dirty
			if (( code == 0 )); then
				unset prompt_pure_custom_git_dirty
			else
				typeset -g prompt_pure_custom_git_dirty="*"
			fi

			[[ $prev_dirty != $prompt_pure_custom_git_dirty ]] && do_render=1

			# When `prompt_pure_custom_git_last_dirty_check_timestamp` is set, the Git info is displayed
			...
			(( $exec_time > 5 )) && prompt_pure_custom_git_last_dirty_check_timestamp=$EPOCHSECONDS
			;;
		prompt_pure_custom_async_git_fetch|prompt_pure_custom_async_git_arrows)
			# `prompt_pure_custom_async_git_fetch` executes `prompt_pure_custom_async_git_arrows`
			# after a successful fetch.
			case $code in
				0)
					local REPLY
					prompt_pure_custom_check_git_arrows ${(ps:\t:)output}
					if [[ $prompt_pure_custom_git_arrows != $REPLY ]]; then
						typeset -g prompt_pure_custom_git_arrows=$REPLY
						do_render=1
					fi
					;;
				99|98)
					...
					;;
				*)
					# Non-zero exit status from `prompt_pure_custom_async_git_arrows`,
	...
	if (( next_pending )); then
		(( do_render )) && typeset -g prompt_pure_custom_async_render_requested=1
		return
	fi

	[[ ${prompt_pure_custom_async_render_requested:-$do_render} = 1 ]] && prompt_pure_custom_preprompt_render
	unset prompt_pure_custom_async_render_requested
}
...
prompt_pure_custom_reset_prompt_symbol() {
	prompt_pure_custom_state[prompt]=${PURE_PROMPT_SYMBOL:-❯}
}

prompt_pure_custom_update_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_pure_custom_state[prompt]=${${KEYMAP/vicmd/${PURE_PROMPT_VICMD_SYMBOL:-❮}}/(main|viins)/${PURE_PROMPT_SYMBOL:-❯}}

	prompt_pure_custom_reset_prompt
}

prompt_pure_custom_reset_vim_prompt_widget() {
	setopt localoptions noshwordsplit
	prompt_pure_custom_reset_prompt_symbol
...
prompt_pure_custom_state_setup() {
	setopt localoptions noshwordsplit

	# Check SSH_CONNECTION and the current state.
	local ssh_connection=${SSH_CONNECTION:-$prompt_pure_custom_SSH_CONNECTION}
	...
	if [[ -z $ssh_connection ]] && (( $+commands[who] )); then
		...
		if [[ $who_out =~ "\(?($reIPv4|$reIPv6|$reHostname)\)?\$" ]]; then
			...
			export prompt_pure_custom_SSH_CONNECTION=$ssh_connection
		fi
		unset MATCH MBEGIN MEND
	fi

	hostname='%F{$prompt_pure_custom_colors[host]}@%m%f'
	# Show `username@host` if logged in through SSH.
	[[ -n $ssh_connection ]] && username='%F{$prompt_pure_custom_colors[user]}%n%f'"$hostname"

	# Show `username@host` if root, with username in default color.
	[[ $UID -eq 0 ]] && username='%F{$prompt_pure_custom_colors[user:root]}%n%f'"$hostname"

	typeset -gA prompt_pure_custom_state
	prompt_pure_custom_state[version]="1.10.3"
	prompt_pure_custom_state+=(
		username "$username"
		prompt	 "${PURE_PROMPT_SYMBOL:-❯}"
	)
}

prompt_pure_custom_system_report() {
	setopt localoptions noshwordsplit
	...
	print - "- Pure state:"
	for k v in "${(@kv)prompt_pure_custom_state}"; do
		print - "\t- $k: \`${(q)v}\`"
	done
...
prompt_pure_custom_setup() {
	...
	zle -N prompt_pure_custom_reset_prompt
	zle -N prompt_pure_custom_update_vim_prompt_widget
	zle -N prompt_pure_custom_reset_vim_prompt_widget
	if (( $+functions[add-zle-hook-widget] )); then
		add-zle-hook-widget zle-line-finish prompt_pure_custom_reset_vim_prompt_widget
		add-zle-hook-widget zle-keymap-select prompt_pure_custom_update_vim_prompt_widget
	fi

	# If a virtualenv is activated, display it in grey.
	PROMPT='%(12V.%F{$prompt_pure_custom_colors[virtualenv]}%12v%f .)'

	# Prompt turns red if the previous command didn't exit with 0.
	PROMPT+='%(?.%F{$prompt_pure_custom_colors[prompt:success]}.%F{$prompt_pure_custom_colors[prompt:error]})${prompt_pure_custom_state[prompt]}%f '

	# Indicate continuation prompt by … and use a darker color for it.
	PROMPT2='%F{242}%_… %f%(?.%F{magenta}.%F{red})${prompt_pure_custom_state[prompt]}%f '

	# Store prompt expansion symbols for in-place expansion via (%). For
	# some reason it does not work without storing them in a variable first.
	typeset -ga prompt_pure_custom_debug_depth
	prompt_pure_custom_debug_depth=('%e' '%N' '%x')
	...
	local -A ps4_parts
	ps4_parts=(
		depth 	  '%F{yellow}${(l:${(%)prompt_pure_custom_debug_depth[1]}::+:)}%f'
		compare   '${${(%)prompt_pure_custom_debug_depth[2]}:#${(%)prompt_pure_custom_debug_depth[3]}}'
		main      '%F{blue}${${(%)prompt_pure_custom_debug_depth[3]}:t}%f%F{242}:%I%f %F{242}@%f%F{blue}%N%f%F{242}:%i%f'
